...
{#if stats.stl_total_acres > 0}
	<p class="font-sans-alt text-sm">
		{#if landUses.length > 1}
			State trust lands on the {$reservation} reservation are primarily used for
			...
			{/each}
		{:else}
			The primary land use on the {$reservation} reservation is {landUses[0].land_use}.
		{/if}
	</p>
	<p class="text-xs font-sans-alt">
		Roughly <span class="font-semibold"
			>{stats.land_uses.uncategorized_acreage.toLocaleString()}</span
		> acres of state trust lands contained no land use information.
	</p>
	<svg
