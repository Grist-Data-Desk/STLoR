...
Generally speaking, in line with state and tribal government practices, we distinguish between surface and subsurface estates and, accordingly, surface and subsurface acreage, including in instances of overlap (i.e. when state trust land encompasses both surface and subsurface rights to a given parcel, we compute acreage sums separately for each set of rights). The Utah Trust Lands Administration (SITLA) informed Grist and High Country News that in such instances of full estate ownership, they dissolve the corresponding acreage: that is, they consider an acre of full-estate ownership to correspond to a single acre of state trust land. This practice diverges from that of all other agencies contacted during our reporting. As such, we apply our acreage-computation methodology uniformly, including in the case of SITLA-owned acreage.

Complete information for all surface and subsurface state trust lands in Oklahoma and some subsurface state trust lands in South Dakota was not obtainable due to data management practices in these states. Some parcels lack digital mapping, and in certain cases, spatial descriptions did not align with the state Public Land Survey System (PLSS), resulting in no returned parcels.

Upon advice from agency sources, we occasionally implemented manual edits to names of trusts that diverged from publicly available datasets. Notably, the North Dakota Commissioner of University & School Lands advised us to rename the Bank of North Dakota beneficiary as the Strategic Investment and Improvements Fund to reflect a previous transfer of the parcels in question to the Department of Trust Lands upon their foreclosure. Furthermore, the Idaho Department of Lands clarified that state trust lands under the purview of our reporting consist of parcels specifically supporting Idaho Endowment Trust beneficiaries (thereby excluding parcels with state-agency or General Fund beneficiaries). We have thus excluded all Idaho parcels for which the benefiting entity is not affiliated with the Idaho Endowment Trust.
